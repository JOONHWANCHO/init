...
옐로우 디자인 시스템, 하단 탭바, 한 손 조작형 입력 UI 등), **2026-07 공통 컴포넌트 중앙화
리팩터링**(`partials.js` 도입으로 내비게이션·푸터·홈 도구 그리드를 단일 소스로 통합), 그리고
**2026-07 익명 댓글 기능**(암호화되어 구글시트에 저장되는 놀이 액티비티·꿀팁 댓글, `play.html`
및 `tips.html` 상세페이지 적용) 내용을 포함해 프로젝트 전체 구조와 운영 방법을 정리한 것입니다.

> **최신 작업(익명 댓글)은 바로 아래 [0. 익명 댓글 기능](#0-익명-댓글-기능-2026-07) 섹션을,**
> **공통 컴포넌트 중앙화는 [0-C. 공통 컴포넌트 중앙화](#0-c-공통-컴포넌트-중앙화-partialsjs-2026-07) 섹션을,**
...
`play.html`의 **액티비티 상세페이지**(놀이 추천 상세보기) 하단 "비슷한 액티비티" 섹션 바로
위, 그리고 `tips.html`의 **꿀팁 상세페이지** 하단 "관련 꿀팁" 섹션 바로 위에 동일한 익명 댓글
기능을 추가했습니다.

- **완전 익명**: 로그인/닉네임/기기 식별 없이 누구나 바로 댓글을 남길 수 있습니다. 어떤 사용자
  식별 정보도 수집하지 않습니다.
- **최대 200자**: 이모지를 포함한 유니코드 문자(코드 포인트) 기준으로 200자까지 입력할 수
  있고, 입력 중 실시간으로 글자 수가 표시되며 초과분은 자동으로 잘립니다. 서버(Apps Script)에서도
  동일한 기준으로 다시 한번 검증합니다.
- **암호화 저장**: 댓글 원문은 브라우저에서 서버로 전송된 뒤, **Apps Script 서버 안에서만
  알고 있는 비밀키로 암호화되어 구글시트에 저장**됩니다. 시트를 직접 열어봐도 암호문만 보이고,
  댓글을 불러올 때만 서버가 복호화해서 돌려줍니다 — 즉 **암호화·복호화 키는 브라우저(프런트엔드
  코드)에 전혀 존재하지 않습니다.**
- **위변조 감지**: 저장된 암호문에는 인증 태그(HMAC)가 함께 붙어서, 누군가 시트를 직접 손으로
  고치면 그 댓글은 조회 시 자동으로 걸러지고 화면에 표시되지 않습니다.
- **기존 톤앤매너 유지**: 세이지 그린 포인트 컬러, 둥근 카드, 스켈레톤 로딩, 눌리는 마이크로
  인터랙션 등 이번에 만든 디자인 시스템을 그대로 사용했습니다.
...
💬 댓글 12                        ← 댓글 수 배지
┌────────────────────────────┐
│ 이 놀이 다녀오셨나요? 익명으로  │  ← 입력창 (200자, 실시간 카운터)
│ 후기를 남겨보세요             │
│ 🔒 완전 익명 · 암호화되어 저장 │
│                    0/200      │
│  [ 익명으로 댓글 남기기 ]      │
...
아이가 정말 좋아했어요 👍
──────────────────────────
🧸 비슷한 액티비티                ← 기존 "비슷한 액티비티" 섹션
[카드] [카드] [카드]
```

댓글 작성자 이름 앞의 동물 이모지·"익명의 OO" 닉네임은 **댓글 id에서 계산한 표시용 장식**일
뿐, 실제 작성자를 추적하거나 구분하기 위한 정보가 아닙니다(서버에는 저장되지 않고, 매번
브라우저가 그 자리에서 계산합니다).

`tips.html`도 동일한 화면 구성입니다 — "관련 꿀팁" 섹션 바로 위에 같은 위젯이 붙습니다.

### 0-3. `activity_id` 네임스페이스 (play와 tips 댓글이 서로 섞이지 않도록)

댓글은 백엔드 시트 한 곳(`comments` 탭)을 `play.html`과 `tips.html`이 함께 씁니다. 그런데 두
콘텐츠의 `id`는 서로 다른 구글시트에서 각각 독립적으로 매겨지는 값이라 **같은 숫자가 우연히
겹칠 수 있습니다**(예: 놀이 id `5`와 꿀팁 id `5`가 동시에 존재). 이 경우를 대비해 프런트엔드에서
`mount()`를 호출할 때 콘텐츠 종류를 접두사로 붙여 넘깁니다.

```js
// play.html
window.NoriyaComments.mount('commentsRoot', 'play-' + item.id);
// tips.html
window.NoriyaComments.mount('commentsRoot', 'tip-'  + tip.id);
```

이렇게 하면 시트에는 `activity_id`가 `play-5`, `tip-5`처럼 저장되어 서로 다른 콘텐츠의
댓글이 절대 섞이지 않습니다. (`comments.gs`의 `list`/`add`는 이 문자열을 그대로 키로 사용할
뿐이라 백엔드 쪽 수정은 필요 없었습니다.) 실제로 jsdom으로 `play-5`에 댓글을 남긴 뒤
`tip-5`를 조회하면 비어 있고, `tip-5`에 남긴 댓글이 `play-5`에는 나타나지 않는 것을
확인했습니다.

### 0-4. 구조 — 프런트엔드 / 백엔드 역할 분담

```
┌─────────────┐   ① 평문 댓글 텍스트 전송 (200자 이하)   ┌──────────────────────┐
│  comments.js │ ───────────────────────────────────▶ │  admin/comments.gs    │
│ (브라우저)   │                                        │  (Apps Script 웹앱)   │
│              │ ◀─────────────────────────────────── │                        │
└─────────────┘   ④ 평문 댓글 목록 반환 (이미 복호화됨)  └──────────┬───────────┘
                                                                   │ ② 서버에서만 아는
                                                        ...
                                                        │ 구글시트 'comments' 탭 │
                                                        │ id·activity_id·iv·    │
                                                        │ ct(암호문)·mac·시각    │  ③ 암호문 상태로 저장
                                                        └──────────────────────┘
```

- **`comments.js`** (신규) — 댓글 위젯 프런트엔드. `play.html`·`tips.html` 양쪽에서 공유합니다.
  목록 조회(`?action=list`), 등록(`?action=add`)을 호출하고, 200자 카운터·스켈레톤 로딩·낙관적
  UI(등록 즉시 목록에 추가) 등을 담당합니다. 댓글 텍스트는 항상 `textContent`로만 렌더링해서
  (절대 `innerHTML`에 사용자 입력을 넣지 않음) 저장형 XSS를 원천 차단합니다.
- **`admin/comments.gs`** (신규) — 댓글 백엔드. `tracker.gs`와 같은 방식(Apps Script 웹앱, GET
  기반 — 이유는 아래 0-6 참고)으로 동작하며, 암호화/복호화, 200자 서버 검증, 시트 읽기/쓰기를
  전담합니다.

### 0-5. 암호화 방식 (AES 없이도 안전하게)

Apps Script에는 AES 같은 대칭키 암호화 함수가 기본 내장돼 있지 않습니다. 그래서 Apps Script에
**내장된** `Utilities.computeHmacSha256Signature`(HMAC-SHA256)만으로 다음과 같은 구성을
사용했습니다 (외부 라이브러리·CDN 의존성 없음, 순수 Apps Script 표준 함수만 사용):

1. 댓글마다 무작위 IV(nonce)를 생성합니다.
2. `키스트림 = HMAC-SHA256(비밀키, IV || 0) || HMAC-SHA256(비밀키, IV || 1) || …` 를 댓글
   길이만큼 이어붙여 만듭니다.
3. `암호문 = 평문 XOR 키스트림` (카운터 모드 스트림 암호와 같은 구성입니다).
4. `인증태그(MAC) = HMAC-SHA256(비밀키, IV || 암호문)` 을 함께 저장합니다 (Encrypt-then-MAC).
5. 복호화할 때는 먼저 MAC을 다시 계산해 저장된 값과 비교하고, 일치하지 않으면(위변조/손상)
   그 댓글은 **조용히 건너뛰고 화면에 보여주지 않습니다.**

비밀키는 **Apps Script "스크립트 속성"(Script Properties)에만 저장**되고, 코드나 시트, GitHub
저장소 어디에도 나타나지 않습니다. `admin/comments.gs`를 처음 배포하고 댓글이 하나라도 등록되면
자동으로 안전한 무작위 키가 생성되어 저장됩니다(수동으로 `initSecretKey()` 함수를 Apps Script
편집기에서 실행해 미리 만들어둘 수도 있습니다).

### 0-6. 검증한 내용

**실제로 실행되는 `admin/comments.gs` 파일 원문을 그대로** Node.js `vm` 모듈에 올리고,
`Utilities`/`PropertiesService`/`SpreadsheetApp`/`ContentService`를 (Google Apps Script의
Java-signed-byte 규칙까지 그대로 재현한) 가짜 객체로 대체해 다음을 직접 실행/확인했습니다.

| 테스트 | 결과 |
|---|---|
| 한글 + 이모지 댓글 암호화 → 저장 → 복호화 조회 | 원문과 정확히 일치 (예: `정말 좋은 놀이 정보네요! 아이가 너무 좋아했어요 😊🎉`) |
| 200자 초과 댓글 등록 시도 | 서버에서 `"댓글은 최대 200자까지 작성할 수 있어요"` 오류로 정상 거부 |
| 빈 댓글 등록 시도 | 서버에서 정상 거부 |
| 댓글 여러 개 등록 후 조회 순서 | 최신순으로 정확히 반환 |
| 서로 다른 `activity_id`끼리 댓글 격리 | 다른 액티비티의 댓글이 섞이지 않음 확인 |
| **위변조 감지** — 시트의 암호문 셀을 직접 임의로 수정 | 해당 댓글만 정확히 걸러지고 나머지는 정상 표시 (MAC 불일치 감지 정상 동작) |
| 내장 `selfTest()` 함수 | 암호화 → 저장 → 조회 → 복호화 한 바퀴 정상 동작 확인 후 자동 정리 |
| `comments.js` 위젯 (jsdom으로 실제 DOM에 렌더링) | `COMMENTS_URL` 미설정 시 "곧 열릴 예정" 안내 + 등록 버튼 비활성화 정상 동작 |
| `comments.js` 위젯 — 가짜 백엔드 연동 | 등록 → 목록 즉시 반영 → 글자수 카운터 실시간 갱신 → 200자 초과 시 자동 잘림 → 새로고침 후에도 목록 유지, 모두 정상 |
| `play-N` / `tip-N` 네임스페이스 격리 | 같은 숫자 id의 `play-5`에 댓글을 남긴 뒤 `tip-5`를 조회하면 비어 있고, 서로의 댓글이 섞이지 않음을 확인 |

### 0-7. 배포 방법 (사이트 운영자용)

1. [script.google.com](https://script.google.com) → 새 프로젝트 (또는 댓글 저장 전용
   스프레드시트를 새로 만들고 **확장 프로그램 → Apps Script**)
2. `admin/comments.gs` 파일 내용 전체를 `Code.gs`에 붙여넣기
3. **배포 → 새 배포 → 웹앱**
   - 다음 사용자로 실행: **나**
   - 액세스 권한: **모든 사용자**
4. 배포된 웹앱 URL을 `config.js`의 `COMMENTS_URL`에 입력
5. (선택) Apps Script 편집기에서 `selfTest` 함수를 선택해 ▶ 실행 → 정상 동작 로그 확인

`COMMENTS_URL`을 비워두면 놀이 상세페이지에는 "댓글 기능이 곧 열릴 예정이에요" 안내만
표시되고, 오류 없이 나머지 기능은 정상 동작합니다 — 즉 **배포 전에도 사이트가 깨지지 않습니다.**

> ⚠ `tracker.gs`와 동일한 이유로 댓글 등록도 `POST`가 아니라 **`GET` + 쿼리 파라미터**
> 방식을 씁니다. Apps Script 웹앱은 요청을 302로 리다이렉트하는데, 브라우저 `fetch`가 이를
> 따라가면서 POST 본문이 사라지고 GET으로 바뀌어버리는 표준 동작 때문입니다. 댓글은 최대
> 200자(한글 기준 URL 인코딩 후 최대 약 1,800자)라 GET 쿼리 길이 제한에 안전하게 들어갑니다.

### 0-8. 새 파일 / 변경 파일 목록

| 파일 | 변경 내용 |
|---|---|
| `admin/comments.gs` (신규) | 댓글 백엔드 — 암호화/복호화, 200자 검증, 시트 읽기/쓰기, `selfTest()` 자체 점검 함수 포함 |
| `comments.js` (신규) | 댓글 위젯 프런트엔드 — 목록 조회/등록, 200자 카운터, 스켈레톤 로딩, 익명 아바타 표시. `play.html`·`tips.html`이 공유 |
| `play.html` | `<script src="comments.js" defer>` 추가, 액티비티 상세 템플릿의 태그 영역과 "비슷한 액티비티" 사이에 `<div id="commentsRoot">` 삽입 + `showDetail()`에서 `NoriyaComments.mount('commentsRoot', 'play-'+item.id)` 호출 |
| `tips.html` | `<script src="comments.js" defer>` 추가, 꿀팁 상세 템플릿의 태그 영역과 "관련 꿀팁" 사이에 `<div id="commentsRoot">` 삽입 + `showDetail()`에서 `NoriyaComments.mount('commentsRoot', 'tip-'+tip.id)` 호출 |
| `style.css` | `.comments-section` 등 댓글 위젯 스타일 추가 (기존 디자인 토큰 재사용) |
| `config.js` | `COMMENTS_URL` 항목 추가 (기본값 빈 문자열 — 안전한 기본 동작) |

### 0-9. 참고 — 이번에 하지 않은 것 (필요하면 확장 가능)

- **신고/삭제/모더레이션 UI**는 포함하지 않았습니다. 필요하다면 `admin/comments.gs`에
  `action=delete`(관리자 비밀번호 확인 후 해당 행 삭제) 같은 액션을 추가하고, `admin/index.html`
  관리자 대시보드에 댓글 관리 탭을 붙이는 방식으로 확장할 수 있습니다.
- **비속어 필터·도배 방지(rate limit)**는 포함하지 않았습니다. 완전 익명이라 IP/기기 기준
  제한은 사용자 추적과 상충할 수 있어 일부러 넣지 않았고, 필요하면 `admin/comments.gs`의
  `addComment()`에 "같은 활동에 최근 N초 내 동일 문구 재등록 차단" 같은 가벼운 규칙을 추가할
  수 있습니다.
- 댓글 기능은 현재 **`play.html`(놀이 액티비티)과 `tips.html`(꿀팁)** 두 곳에 적용돼 있습니다.
  다른 콘텐츠(예: 도구 페이지 결과 화면)에도 붙이고 싶다면, 동일한 패턴(`<div id="commentsRoot">`
  + `NoriyaComments.mount('commentsRoot', '접두사-' + id)` 호출)을 그대로 재사용하면 됩니다 —
  다른 콘텐츠 종류와 id가 섞이지 않도록 0-3에서 설명한 것처럼 **콘텐츠별 접두사를 꼭 붙여주세요.**

---

## 0-C. 공통 컴포넌트 중앙화 (`partials.js`, 2026-07)
...
```html
<!-- 이전: <footer>© 2026 아이로그 · <a href="../index.html">전체 도구 보기</a></footer> -->
<!-- 이후 (도구 페이지) -->
