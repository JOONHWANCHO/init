...
| **SEO** | 전 페이지에 `canonical`, Open Graph/Twitter 카드 보강, `robots` 메타, `JSON-LD` 구조화 데이터(WebSite/Organization/BreadcrumbList/ItemList/WebApplication/CollectionPage) 추가. `sitemap.xml`에 누락돼 있던 `play.html`/`tips.html` 추가 및 `lastmod` 갱신. `robots.txt`에 `/admin/` 차단 추가 |
...
> 자세한 원리는 [9. `play.html` 성능 개선 상세](#9-playhtml-성능-개선-상세) 참고.
...
```
.
├── index.html               # 홈 — 도구 18종 그리드, 카테고리별 정리, 검색
├── play.html                 # 놀이 추천 — 목록/상세 SPA형 페이지 (Sheet2 연동)
├── tips.html                 # 육아 꿀팁 — 목록/상세 SPA형 페이지 (Sheet1 연동)
├── style.css                  # 전역 스타일시트 (홈/놀이/꿀팁 공통, 도구 페이지는 자체 <style> 포함)
